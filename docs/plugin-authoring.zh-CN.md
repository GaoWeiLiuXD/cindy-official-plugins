...
插件开发不应等待 Cindy 客户端能力发布。在支持的清单格式内，不认识的能力声明是
数据，不应成为发布或安装阻塞。校验器保留未知顶层字段、能力对象扩展字段
（mainView、panel、card、agent、node 及凭证绑定、preview、skill/manual 及其条目、
subscribe）、Cindy 类目/动作与订阅 topics/hooks；旧 v2 slot 名称也不受仓库私有能力
白名单限制。

接受声明不等于实现接口或授予权限。已知字段类型、安全包路径、格式约束，以及运行时
账号、凭证和会话授权仍然生效。未知声明在 Host 实现前不产生能力；凭证引用无法解析时，
应在使用时报错，不能自动改用其它凭证或账号。

作者必须检测 API 是否存在，或处理“不支持此操作”的结果，只禁用受影响的功能，
明确提示升级或使用已有能力降级；不能绕过拒绝授权，也不能静默报告成功。
`minCindyVersion` 仍按规则用于分发和版本选择，但不是运行时支持证明：用户仍可能在
不适配的客户端直接安装插件。应覆盖这种情况，保留其它可用功能。

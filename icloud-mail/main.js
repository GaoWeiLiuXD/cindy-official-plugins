 ...
 * iCloud Mail 官方插件。
 *
 * App 专用密码由设置页直接写入 Cindy 的 /secrets 只写通道。浏览器 main.js 只处理
 * 非敏感邮箱地址；宿主仅在 ghost.json 绑定的 Node 方法上，把密码临时注入
...
function normalizeEmail(value) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254) {
    throw new Error('请输入有效的 iCloud Mail 邮箱地址');
  }
  return email;
}
...
async function readJson(path) {
  ...
  if (!response.ok) throw new Error('读取 iCloud Mail 配置失败');
...
function normalizeCredentialSlot(value) {
  ...
  if (value !== 'a' && value !== 'b') {
    throw new Error('iCloud Mail 凭证槽位无效，请重新连接');
  }
  return value;
}
...
async function requireConfiguredAccount() {
  ...
  if (!state.connected || !state.email) {
    throw new Error('尚未配置 iCloud Mail，请到「iCloud Mail」插件详情页完成连接');
  }
    ...
    void handleSettingsRequest(message.action, payload)
      ...
      .catch(function failure(error) {
        return {
          ...
          reqId: message.reqId,
          ok: false,
          message: error && error.message ? error.message : 'iCloud Mail 连接失败，请重试',
        };
      })
...
async function runMail(args) {
  ...
  try {
    ...
    return fail(error && error.message ? error.message : 'iCloud Mail 参数无效');
  }
  ...
  try {
    ...
    return fail(error && error.message ? error.message : 'iCloud Mail 操作失败，请稍后重试');
  }
}
...
async function statusResult() {
  try {
    ...
    return fail(error && error.message ? error.message : 'iCloud Mail 连接状态查询失败');
  }
}
...
cindy.onHostMessage(function onHostMessage(message) {
  ...
  void (async function dispatch() {
    try {
      var result = message.tool === 'icloud_mail_status'
        ? await statusResult()
        : message.tool === 'icloud_mail'
      ...
      await cindy.send({
        ...
        callId: message.callId,
        ok: false,
        message: error && error.message ? error.message : 'iCloud Mail 工具执行失败',
      });
    }
  })();
});
